...
fetchint(struct proc *p, uint addr, int *ip)
{
  //if(addr >= p->sz || addr+4 > p->sz || addr == 0 )
	if(((addr < ((int)p->botStack)) && (addr >= p->sz || addr+4 > p->sz)) || ((addr < PGSIZE) && proc->pid != 1) || addr >= USERTOP || addr+4 > USERTOP) //(addr >= p->sz || addr+4 > p->sz) && //|| addr == 0 )
	{
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  //if(addr >= p->sz || addr == 0)
  if(((addr < ((int)p->botStack)) && (addr >= p->sz)) || ((addr < PGSIZE) && proc->pid != 1) || addr > USERTOP)
	{
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
	...
	if((((uint)i >= proc->sz || (uint)i+size > proc->sz) && ((uint) i < ((int)proc->botStack)))|| ((i < PGSIZE) && proc->pid != 1) || (uint)i >= USERTOP || (uint)i+size > USERTOP)
	{
